...
sap.ui.commons.DatePicker.prototype.onAfterRendering = function(){
	...
	jQuery(oInput).bind('keypress', jQuery.proxy(sap.ui.commons.DatePicker.prototype._handleKeypress, this));

	var oBindingInfo = this.getBindingInfo("value");
	var sLocale   = this.getRenderedLocale();
	// in mobile mode don't attach the jQuery datePivker to the field
	...
	if (!this.mobile){
		...
		if (oBindingInfo && oBindingInfo.type && oBindingInfo.type instanceof sap.ui.model.type.Date) {
			var sPattern = oBindingInfo.type.getOutputPattern();
			oSelector.datepicker( "option", "dateFormat", this._convertDatePattern(sPattern));
		}
		this.pattern = oSelector.datepicker( "option", "dateFormat"); // store pattern to check input
	}else{
		// as only after rendering its really clear what pattern is used store it
		// on mobile devices output pattern is yyyy-mm-dd but for value property it's the one of the locale
		this.pattern = jQuery.datepicker.regional[sLocale].dateFormat;
		if (oBindingInfo && oBindingInfo.type && oBindingInfo.type instanceof sap.ui.model.type.Date) {
			this.pattern = this._convertDatePattern(oBindingInfo.type.getOutputPattern());
		}
	}
	this.oNames = {dayNamesShort : jQuery.datepicker.regional[sLocale].dayNamesShort,
				   dayNames : jQuery.datepicker.regional[sLocale].dayNames,
				   monthNamesShort : jQuery.datepicker.regional[sLocale].monthNamesShort,
				   monthNames : jQuery.datepicker.regional[sLocale].monthNames};
...
sap.ui.commons.DatePicker.prototype._handleKeypress = function(oEvent){
	...
	if (String.fromCharCode(iCharCode) == "+" && !oEvent.ctrlKey && !oEvent.shiftKey) {
		// increase by one day
		this._increaseDate("day", 1, oEvent);
	}else if (String.fromCharCode(iCharCode) == "-" && !oEvent.ctrlKey && !oEvent.shiftKey) {
		// decrease by one day
		this._increaseDate("day", -1, oEvent);
	}else if (String.fromCharCode(iCharCode) == "+" && !oEvent.ctrlKey && oEvent.shiftKey) {
...
sap.ui.commons.DatePicker.prototype.setYyyymmdd = function(sYyyymmdd) {
	...
	try{
		var oDate = jQuery.datepicker.parseDate('yymmdd', sYyyymmdd);
		var oSelector = jQuery.sap.byId(this._getInputId());
		var sValue;
		if (this.mobile && this.pattern) {
			// on mobile devices the date is displayed in native pattern, so it must be
			// converted into control pattern fpr value property
			sValue = jQuery.datepicker.formatDate( this.pattern, oDate, this.oNames);
			var sOutputValue = jQuery.datepicker.formatDate( "yy-mm-dd", oDate );
...
sap.ui.commons.DatePicker.prototype.setValue = function(sValue) {
	...
	try{
		...
		if (this.mobile && this.pattern) {
			// on mobile devices the date is displayed in native pattern, so it must be
			// converted into control pattern fpr value property
			oDate = jQuery.datepicker.parseDate(this.pattern, sValue, this.oNames);
			sValue = jQuery.datepicker.formatDate( "yy-mm-dd", oDate );
			oSelector.val(sValue);
		}else{
		...
		var sYyyymmdd = jQuery.datepicker.formatDate( 'yymmdd', oDate );
...
sap.ui.commons.DatePicker.prototype.setLocale = function(sLocale) {
	...
	if(!this.mobile){
		...
		var oDate = jQuery.datepicker.parseDate(this.pattern, sValue, this.oNames);
		this.pattern = jQuery.datepicker.regional[sLocale].dateFormat;
		this.oNames = {dayNamesShort : jQuery.datepicker.regional[sLocale].dayNamesShort,
					   dayNames : jQuery.datepicker.regional[sLocale].dayNames,
					   monthNamesShort : jQuery.datepicker.regional[sLocale].monthNamesShort,
					   monthNames : jQuery.datepicker.regional[sLocale].monthNames};
		var sValue = jQuery.datepicker.formatDate( this.pattern, oDate, this.oNames);
		this.setProperty("value", sValue, true);
	}
...
sap.ui.commons.DatePicker.prototype._checkChange = function(oEvent) {
	...
	if (this.mobile && this.pattern) {
		// on mobile devices the date is displayed in native pattern, so it must be
		// converted into control pattern fpr value property
		var oDate = jQuery.datepicker.parseDate('yy-mm-dd', newVal);
		newVal = jQuery.datepicker.formatDate( this.pattern, oDate, this.oNames);
	}
	...
	if(this.getEditable() && this.getEnabled() && newVal != this.getValue() && newVal != this.getYyyymmdd()) {
		if (newVal.length==8 && parseInt(newVal, 10)==newVal){
			...
			var bWrong = false;
			try{
				oDate = jQuery.datepicker.parseDate(this.pattern, newVal, this.oNames);
			} catch (e) {
				// fits not to pattern - try standard pattern
				bWrong = true;
			}
			// check if locale short format is used!!!
			if (bWrong) {
				bWrong = false;
				var oLocale = new sap.ui.core.Locale(sLocale);
				var oLocaleData = sap.ui.core.LocaleData.getInstance(oLocale);
				var sLocalePattern = this._convertDatePattern(oLocaleData.getDatePattern("short"))
				try{
					oDate = jQuery.datepicker.parseDate(sLocalePattern, newVal, this.oNames);
					newVal = jQuery.datepicker.formatDate( this.pattern, oDate, this.oNames);
				} catch (e) {
					// fits not to pattern - try standard pattern
					bWrong = true;
