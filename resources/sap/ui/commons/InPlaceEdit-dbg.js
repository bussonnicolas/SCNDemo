 ...
 * <li>{@link #getUndoEnabled undoEnabled} : boolean (default: true)</li></ul>
 * </li>
 * <li>Aggregations
 * <ul>
 ...
 * is always focused. The focusout handling must be done vith a delay because in ComboBoxes the click og the expand button fires a focusout event too. 
 * So it must be checked if the focus is still inside the control.
 * As the content controld don't bubble their property changes the _change event is handled. Because the InPlaceEdit have to react on content-changes.
...
(function() {
	...
	sap.ui.commons.InPlaceEdit.prototype.onAfterRendering = function(){
		...
		if (this._bEditMode) {
			...
			jQuery.sap.byId(this._oDisplayControl.getId()).css("width", "auto");
			jQuery.sap.byId(this._oDisplayControl.getId()).css("max-width", "100%");
		}else{
			jQuery.sap.byId(this._oDisplayControl.getId()).css("width", "100%");
		}

		// if undo button is rendered remove it from tab-chain
		if (this._sOldText && this._oUndoButton && jQuery.sap.domById(this._oUndoButton.getId())) {
 ...
 */
	...
	function updateControls(oInPlaceEdit){
		...
		case "sap.ui.commons.DropdownBox":
			...
			oInPlaceEdit._oTextView.setDesign(oContent.getDesign());
	...
	function switchToEditMode(oInPlaceEdit){
		...
		if (!oInPlaceEdit._bEditMode && oInPlaceEdit.getEditable()) {
			...
			if (!oInPlaceEdit._sOldText && oInPlaceEdit.getUndoEnabled()) {
	...
	function undoTextChange(oInPlaceEdit){
		...
		if (oInPlaceEdit.getUndoEnabled() && oInPlaceEdit._sOldText) {
			var oContent = oInPlaceEdit.getContent();
			if (!oContent) {
				return;
			}
