...
sap.ui.commons.InPlaceEditRenderer.render = function(rm, oInPlaceEdit){
	...
	if (oInPlaceEdit.getUndoEnabled() && oInPlaceEdit._sOldText && ( !oInPlaceEdit._bEditMode || ( oInPlaceEdit._bEditMode && oInPlaceEdit._oEditControl.getValue() != oInPlaceEdit._sOldText))) {
	...
	rm.writeStyles();
	rm.write(">"); // DIV
	if (oInPlaceEdit._sOldText || oContent.getMetadata().getName() == "sap.ui.commons.Link") {
	...
	if (oInPlaceEdit.getUndoEnabled() && oInPlaceEdit._sOldText) {
		rm.write("</DIV>");
		// there is an old text available - render undo button
