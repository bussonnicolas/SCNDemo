...
sap.ui.commons.Carousel.prototype.showPrevious = function() {
	...
	$ScrollList.find('li:last').insertBefore($ScrollList.find('li:first'));
	var me = this;
...
sap.ui.commons.Carousel.prototype.calculateAndSetSize = function() {
	...
	if (aContent.length > 0) {
		...
		for ( var i = 0; i < aContent.length; i++) {
			var childWidth = aContent[i].getWidth();
			var childHeight = aContent[i].getHeight();
		...
		jQuery.sap.byId(this.getId()).addClass(
				'sapUiCrsl'
						+ jQuery.sap.charToUpperCase(this.getOrientation(), 0));
		...
		if (this.getOrientation() == "horizontal") {
			...
			$ContentArea.find('.sapUiCrslItm').width(maxWidth)
					.height(maxHeight);
			var cLineHeight = maxHeight + 10 + "px";
			$PrevButton.css("height", maxHeight + 10).css("line-height",
					cLineHeight);
			$NextButton.css("height", maxHeight + 10).css("line-height",
					cLineHeight);
			$ContentArea.width(contentBarSize).height(maxHeight + 10);
		} else {
			...
			$ContentArea.find('.sapUiCrslItm').width(maxWidth)
					.height(maxHeight);
			$PrevButton.width(maxWidth + 10).after($ContentArea);
			$NextButton.width(maxWidth + 10);
			$ContentArea.width(maxWidth + 10).height(contentBarSize);
		}
	}
};
