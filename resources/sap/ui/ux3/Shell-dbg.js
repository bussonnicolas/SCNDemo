...
sap.ui.ux3.Shell.prototype.onUserActivation = function(oEvent) {
	...
	if (oEvent.target.className 
		...
		if (parent && parent.className && parent.className.indexOf("sapUiUx3ShellPaneEntries") > -1) {  // pane bar item clicked
...
(function() {
	...
	sap.ui.ux3.Shell.prototype._closePane = function(fCompleteCallback) {
		...
		this._getSyncRefs().stop().animate(props);
	...
	sap.ui.ux3.Shell.prototype._openPane = function() {
		...
		if (this._bRtl) {
			...
			this._getSyncRefs().stop().animate({left: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
			...
			this._getSyncRefs().stop().animate({right: (PANE_WIDTH + sap.ui.ux3.Shell._PANE_OUTER_DISTANCE + sap.ui.ux3.Shell._SHELL_OFFSET_RIGHT) + "px"});
	...
	sap.ui.ux3.Shell.prototype.setOffsetRight = function(px, complete, outerId) {
		...
		jQuery.sap.byId(id+"-hdr").stop().animate({right: pxVal});
		jQuery.sap.byId(id+"-hdrImg").stop().animate({right: pxVal});
		jQuery.sap.byId(id+"-bg").stop().animate({right: pxVal}, function() {
			if(complete){
				complete();
			}
		});
		
		jQuery.sap.byId(id+"-bgImg").stop().animate({right: pxVal});
		jQuery.sap.byId(id+"-wBar").stop().animate({right: pxVal});
		jQuery.sap.byId(id+"-paneBar").stop().animate({right: pxVal});
		...
		jQuery.sap.byId(id+"-notify").stop().animate({right: pxVal});
		this._getSyncRefs().stop().animate({right: pxVal});
		jQuery.sap.byId(id+"-canvas").stop().animate({right: pxVal});
		jQuery.sap.byId(id+"-canvasBackground").stop().animate({right: pxVal});
	};
...
sap.ui.ux3.Shell.prototype.syncWithCanvasSize = function(sId, bInit, fFocusFirst, fFocusLast) {
	...
	if(this._aSyncRefStack.length > 0){
		...
		$DomRef.css("left", oCanvas.offsetLeft + "px");
		$DomRef.css("right", (jQuery(document).width() - oCanvas.offsetLeft - oCanvas.offsetWidth) + "px");
