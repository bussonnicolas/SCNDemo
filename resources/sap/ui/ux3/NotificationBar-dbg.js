...
(function() {
	...
	var fnSettingWidth = function(that) {
		if (that.getMessageNotifier() && that.getMessageNotifier().hasItems()) {
			...
			if ($domRef.length > 0) {
				...
				if ($messageArea) {
					// -2 since the IE doesn't display the inplace message in
					// the demokit
	...
	var fnMouseMoveListener = function(oEvent) {
		...
		var $hoverDomRef = jQuery(hoverDomRef);
	...
	var fnHover = function(oEvent) {
		...
		var $toggler = jQuery(jQuery.sap.byId(sId));
	...
	sap.ui.ux3.NotificationBar.prototype.onfocusin = function(oEvent) {
		...
		var bNotifier = ($target.hasClass("sapUiNotifier")) ? true : false;
		var sStatus = this.getVisibleStatus();

		if (!bNotifier && sStatus !== "Min") {
			/*
